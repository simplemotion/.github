...
|---------|------|------|--------|---------|
| — | — | 2026-08-22 | Greg Gowans | **Add the sm-pr caller stub, so the PR gate can report its checks.** This repo carried no sm-pr.yml, so it emitted neither the pr slash meta nor the pr slash Repo hygiene context. 76 of the 86 org-default repos across the enterprise were in that state - carrying sm-ci but not sm-pr, so nothing ever validated a pull request. That is why two separate defects went unseen in these repos until today: 32 were missing SECURE.md entirely, a HARD hygiene failure, and 44 more carried a security policy headed after the template they were copied from. No gate, no finding. Copied verbatim from 3400-9992-SM-PR examples caller-stub.yml including the commented with: block, whose opt-out lines stay commented - uncommenting one is for the rare repo where a fleet-wide assertion genuinely does not apply. The stub lands FIRST and has_pr_gate follows only once this repo is proven to report, because a repo with no stub cannot report the checks its ruleset requires and none of its PRs could ever merge. |
