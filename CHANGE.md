...
|---------|------|------|--------|---------|
| (auto) | &mdash; | 2026-08-28 02:00 UTC | Greg Gowans | **Call the shared public-safe gate instead of carrying an inlined copy of it.** The 298-line sm-pr-inline.yml is replaced by the same 24-line stub every public repo now uses. The logic did not change: the shared workflow in this repo was lifted from this very file, so enforcement is identical, plus the SECURE.md heading check the inlined copies never had. This is the repo that hosts the shared workflow, so it calls its own - which is the point, since a public repo can call a public reusable workflow and that is the whole reason the shared gate can exist at all. |
